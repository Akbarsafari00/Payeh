...
namespace Payeh.ApplicationService.Queries
{
    ...
    public class QueryDispatcher : IQueryDispatcher
    {
        ...
        public Task<QueryResult<TData>> Execute<TQuery, TData>(TQuery query) where TQuery : class, IQuery<TData>
        {
            ...
            {
                _stopwatch.Stop();
                _logger.LogInformation(HaniaEventId.PerformanceMeasurement, "Processing the {QueryType} command tooks {Millisecconds} Millisecconds", query.GetType(), _stopwatch.ElapsedMilliseconds);
            }
